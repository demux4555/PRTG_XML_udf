...
; #DESCRIPTION# =================================================================================================================
; Title .........: PRTG_XML_udf v3.1
; Description ...: UDF to allow easier output of PRTG channels in XML format.
; Author(s) .....: demux4555
; Changelog .....:
;	v3.0	2019.11.19	Proper code cleanup.
;   v3.1	2019.12.05	Added _PrtgReturnError()
...
#Region ### CONSTANTS FOR PRTG XML TAGS ###
	...
	Global Const $PRTG_ERROR_PRINTBOTH 			= 1
	Global Const $PRTG_ERROR_PRINTSTDERRONLY 	= 2
	Global Const $PRTG_ERROR_NOEXIT 			= "[NOEXIT]"
	...
	; PRTG Custom Sensor bit combos commonly used.
	; These are mostly for convenience. Some are double up, due to backwards compatibility (or changes) with older scripts. Edit at own convenience.
	Global Const $PRTG_CS_HIDE			= $PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO																; hidden
	Global Const $PRTG_CS_PERCENT 		= $PRTG_UNIT_PERCENT+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_AUTO+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO	; percentage (%) hidden
	Global Const $PRTG_CS_PCT 			= $PRTG_UNIT_PERCENT+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_AUTO 										; percentage (%)
	...
	Global Const $PRTG_CS_CPULA 		= $PRTG_UNIT_CUSTOM+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_ALL+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO		; cpu load average (la) hidden
	Global Const $PRTG_CS_HZ 			= $PRTG_UNIT_CUSTOM+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_ALL+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO		; frequency/hertz (Hz) hidden
	Global Const $PRTG_CS_VAC 			= $PRTG_UNIT_CUSTOM+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_ALL+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO		; voltage (VAC) hidden
	...
	Global Const $PRTG_CS_SECONDS_CHART	= $PRTG_UNIT_TIMESECONDS+$PRTG_SHOWTABLE_NO 														; seconds
	Global Const $PRTG_CS_MEM 			= $PRTG_UNIT_BYTESMEMORY																			; memory used (bytes)
	Global Const $PRTG_CS_MEM_HIDE		= $PRTG_UNIT_BYTESMEMORY+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO										; memory used (bytes) hidden
	...
	Global Const $PRTG_CS_EXECTIME		= $PRTG_UNIT_CUSTOM+$PRTG_FLOAT_YES+$PRTG_DECIMALMODE_AUTO+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO	; Sensor Execution Time (custom, sec) hidden
	...
	Global Const $PRTG_CS_OVL			= $PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO 															; lookup hidden
	Global Const $PRTG_CS_OVL_FLOAT		= $PRTG_FLOAT_YES+$PRTG_SHOWCHART_NO+$PRTG_SHOWTABLE_NO 											; lookup float hidden
	Global Const $PRTG_CS_DONTPRINTCHANNEL = $PRTG_DONTPRINT																				; hidden
...
#Region ### XML OUTPUT ###
	...
	; #FUNCTION# ====================================================================================================================
	; Name ..........: _PrtgReturnError
	; Description ...: Show an XML PRTG error to put sensor into error mode.
	; Syntax ........: _PrtgReturnError([$_sMessageText = ""[, $_iExitCode = 0[, $_OUTPUT = $PRTG_ERROR_PRINTBOTH]]])
	; Parameters ....: $_sMessageText       - [optional] Text to display in Error element.
	;                  $_iExitCode          - [optional] Integer. The Exit code to use when exiting the script. If set to
	;                                                    $PRTG_ERROR_NOEXIT, the function will not Exit after error is printed.
	;                                                    Default is 0.
	;                  $_OUTPUT             - [optional] Allows printing $_sMessageText as XML only, to STDERR only, or both.
	;                                                    Default is $PRTG_ERROR_PRINTBOTH (1).
	; Return values .: None
	; Author ........: demux4555
	; ===============================================================================================================================
	Func _PrtgReturnError($_sMessageText = "", $_iExitCode = 0, $_OUTPUT = $PRTG_ERROR_PRINTBOTH)
		...
		; ---- EXIT AND EXIT CODE ----
		Select
			Case StringIsInt($_iExitCode)									; allow signed integer values
				Exit Number($_iExitCode)									; ... and use integer as Exit code
			Case ($_iExitCode=="")											; if empty "" we don't apply an Exit code
				Exit
			Case ($_iExitCode==$PRTG_ERROR_NOEXIT) Or ($_iExitCode==Null)	; we don't Exit the script
				Return
			Case Else
				; dummy line
		EndSelect
